...
already was. It applied tier 1, the driving port, and deferred tiers 2 and 3.
The deferral rested on one specific cost. It is worth restating precisely,
...
along unexamined. Drop that assumption, keep `domain` and `app` importable, and
the facade has nothing to do. A library user imports the two packages directly.
No alias needs to exist, now or for every type added later.
...
with an initial commit, no release and no external consumers. It priced the move
at two to three days of import rewriting. That price only goes up. Every week of
new code is more files to move and, eventually, an API someone depends on. The
layout is free exactly once.
...
0008 named the whole of the work: five unexported symbols crossing the boundary,
namely `normalizeTime`, `canonicalJSON`, `writeChunk`, `validateLedgerID` and
`postingsToWire`. It observed that four of the five were reached through
`fingerprint()`. That function hashes a command to decide whether an idempotency
key is a replay or a misuse. Its recommendation:
...
optional timestamp and an optional id now live in `domain/fingerprint.go`. Three
functions sit in front of them: `FingerprintOpenAccount`, `FingerprintCommit`
and `FingerprintRevert`. Command identity is a rule about what makes two
requests the same request, which is the ledger's business and not the service's.
...
Both were already documented domain rules with an exported constant beside them
(`TimePrecision`, `MaxLedgerIDLen`).
...
function that produces it. A domain rule derives the read model from the log, so
the read model's shape is domain-shaped. The second is that `BalanceQuery`
encodes this project's whole thesis, including the rule that `AsOfSeq` wins over
...
vocabulary of failure that adapters match with `errors.Is`. Split across two
packages, it would make every caller import both in order to handle errors, and
it buys nothing.
...
tree shows where each role lives, and this document no longer has to serve as
the map.
...
held in place by the conformance suite running against both stores, by the
property tests, and by a new golden test that pins the three command
fingerprints. The fingerprints are the one place in the move where a mistake
would have been silent rather than a build failure.
