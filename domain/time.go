...
// timestamptz holds microseconds. Suppose the ledger hashed a
// nanosecond-precision time and then read it back from the database. The
// recomputed hash would not match the stored one, and the chain would look
// broken. Truncating on the way in makes an event's hash survive a round trip
// through storage.
