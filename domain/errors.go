...
// [errors.Is]. Every error this package produces wraps one of these, so the
// HTTP layer can map a failure to a status code without a type switch.
var (
	...
	// ErrOverflow reports a monetary value outside the representable range. See
	// [Amount] for the limits.
...
var (
	...
	// ErrInsufficientFunds reports a posting that would push an account past zero
	// in its normal direction when it does not allow that.
...
var (
	...
	// ErrUnknownEvent reports an event whose type or payload this build does not
	// understand.
...
// Command errors.
var (
	// ErrIdempotencyConflict reports an idempotency key reused with a different
	// command. The first outcome stands. The ledger will not guess which of the
	// two the caller meant.
	ErrIdempotencyConflict = errors.New("ledger: idempotency key reused with a different request")

	// ErrTransactionNotFound reports a reference to a transaction the ledger has
	// never recorded.
	...
	// ErrConflict reports that another writer advanced the stream first and the
